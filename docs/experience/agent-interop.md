...
`graphforge.runtime` (default `auto`) selects which engine backs Cypher/verb calls — see issue [#12](https://github.com/CurateLabs/graphforge-vscode/issues/12) and the root `README.md`'s "Runtimes: Node vs Python" section for the full policy. **Node remains the default and only backs the advanced Phase 0–4 surfaces** (checkpoints, embedding spaces, indexing, invocation descriptors, composite transactions, and knowledge-ledger writes); the Python bridge is a first-class alternative for `execute` and the analyst verbs only. `graphforge.checkEnvironment`'s `EnvironmentReport` reflects both runtimes independently (`nodeBinding`, `python`) plus which one is actually backing the open session (`runtime.active`), so an agent can tell "Node is default but Python is what's active" apart from "neither is usable." `feat/12-python-prefer-uv` has since merged: `auto` now classifies the workspace as Python-first / Node-ish / ambiguous (`projectKindDetector.ts`) and prefers Python in Python-first workspaces even when Node is available, and `Setup Python Binding` installs exclusively via `uv` (never `pip`) — see FR-16/FR-18 in `docs/REQUIREMENTS.md`.
